...
// @version      6.7
// @description  Сканер ГЖЛТ. Без дублей в сохранении, оптимизированный таймаут, чистый заголовок.
...
(function () {
        ...
        for (const ticket of tickets) {
            ...
            if (checkSingleRow) {
                ...
                for (let i = 0; i < allSpans.length; i += 9) {
                    ...
                    rowSpans.forEach(s => { const n = parseInt(s.textContent.trim(), 10); if (!isNaN(n)) rowNumSet.add(n); });
            ...
            allSpans.forEach(span => {
                ...
                if (!isNaN(val) && matched.includes(val)) { span.setAttribute('data-slt-hit', '1'); span.classList.add('slt-num-hit', isFull ? 'full' : 'partial'); }
            });
            ...
            if (matchCount >= stopThreshold) { stopReason = checkSingleRow ? `≥${stopThreshold} в одной строке` : `≥${stopThreshold} совпадений`; stoppedTicket = ticket; break; }
        }
    ...
    function triggerLoadMore() {
        ...
        if (!loadBtn || isWaitingForLoad) return;

        isWaitingForLoad = true;
        const body = document.getElementById('slt-results-body');
        if (body) {
            const loadMsg = document.createElement('div');
            loadMsg.className = 'slt-loading';
            loadMsg.textContent = '⏳ Загрузка других билетов...';
        ...
        const observer = new MutationObserver((mutations, obs) => {
            ...
            if (body && body.querySelector('.slt-loading')) body.querySelector('.slt-loading').remove();
        });
        observer.observe(document.body, { childList: true, subtree: true });
        timeoutId = setTimeout(() => { observer.disconnect(); isWaitingForLoad = false; if (body && body.querySelector('.slt-loading')) body.querySelector('.slt-loading').remove(); }, 4000);
    }
    ...
    function renderResults(container, results) {
        ...
        results.forEach((r, i) => {
            ...
            tr.innerHTML = `
                <td style="padding:7px 6px 7px 10px;color:${THEME.textMuted};font-size:11px">${i+1}</td>
                <td style="padding:7px 6px"><span style="font-weight:700;font-size:12px;color:${r.isFull?THEME.highlight:THEME.text}">${r.ticketId}</span>${r.isFull?`<span style="margin-left:5px;font-size:9px;font-weight:700;background:${THEME.highlight};color:${THEME.bgDark};padding:1px 5px;border-radius:4px">★ ПОЛНОЕ</span>`:''}${i===0?`<span style="margin-left:5px;font-size:9px;font-weight:700;background:${THEME.primary};color:${THEME.bgDark};padding:1px 5px;border-radius:4px">👑 ТОП</span>`:''}</td>
                <td style="padding:7px 6px;text-align:center"><span style="background:${THEME.bgDarker};color:${countColor};border:1px solid ${countColor}66;padding:2px 10px;border-radius:10px;font-weight:700;font-size:12px">${r.count}/${selectedNumbers.size}</span></td>
                <td style="padding:7px 10px 7px 6px;line-height:2">${numsHtml}</td>
                <td style="padding:7px 6px;text-align:center"><button class="slt-save-btn" title="Сохранить результат" data-idx="${i}">💾</button></td>
            `;
    ...
    function cleanupHighlights() { document.querySelectorAll(`${SEL.NUMBER}[data-slt-hit]`).forEach(s => { s.removeAttribute('data-slt-hit'); s.classList.remove('slt-num-hit', 'full', 'partial'); }); document.querySelectorAll('.slt-ticket-best, .slt-ticket-selected').forEach(el => el.classList.remove('slt-ticket-best', 'slt-ticket-selected')); }
    ...
    function doScan() {
        ...
        if (payload.results.length) payload.results[0].ticket.scrollIntoView({ behavior: 'smooth', block: 'center' });
        if (!payload.stopReason && autoLoadEnabled && isScanning) triggerLoadMore();
    }
